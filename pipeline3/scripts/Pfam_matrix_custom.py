...
parser.add_argument( 
    "--homology_type",
    help="either orthologs, paralogs, orthologs",
    type=str
)
...
samples_df = pd.read_csv(args.samples_dir + "/" + args.species + "_" + args.homology_type + "_samples.csv")


# read in the samples for both the species and the candidate homologies
with open( args.samples_dir + "/" + args.species + "_" + args.homology_type + "_sample_neighbour.json","r") as f:
    sample_neighbours = json.load(f)
    
with open( args.samples_dir + "/" + args.species + "_" + args.homology_type + "_homology_sample_neighbour.json","r") as f:
...
def overlap(tup):
    ...
    # try:
    # print(args.Pfam_dir + "/" + species + "/" + gene1 + "_" + species + ".outs.tsv.gz")
    # print(args.Pfam_dir + "/" + homology_species + "/" + gene2 + "_" + homology_species + ".outs.tsv.gz")
    # print(type(gene1))
    # print(type(gene2))
    try:
        temp_df1 = pd.read_csv( args.Pfam_dir + "/" + species + "/" + gene1 + "_" + species + ".outs.tsv.gz", compression="gzip", delimiter="\t", names=keys,usecols=[0,2,6,7])
    ...
    if (temp_df1.shape[0] == 0) or (temp_df2.shape[0] == 0):
        ...
        # Get the jaccard index of the overlaps
...
np.save(args.out_dir + "/" + args.species + "_" + args.homology_type + "_Pfam.npy", stacked_pfam) # save the stacked pfam as an npy
