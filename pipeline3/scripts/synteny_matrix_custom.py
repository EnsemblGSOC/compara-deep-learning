...
# concatenate the matrices into one bigger one
stacked_synteny = np.concatenate((global1[:,:,:,np.newaxis],global2[:,:,:,np.newaxis],local1,local2), axis = -1)
print(stacked_synteny.shape)
print("File saved to: " + args.out_dir + "/" + args.species + "_" + args.homology_type + "_synteny.npy")
np.save(args.out_dir + "/" + args.species + "_" + args.homology_type + "_synteny.npy", stacked_synteny)
